...
async def find_friends_by_books(msg: types.Message):
    ...
    matches_info[user["_id"]] = tuple([(key, matches[key]) for key in matches.keys()])
...
async def add_existing_friend(msg: types.Message):
    query_name = msg.text
    if query_name in [m[0] for m in matches_info[msg.from_user.id]]:
